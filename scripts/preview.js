...
(async () => {
  ...
  const lcov = hasLcov ? parseLcov(root, fs.readFileSync(lcovPath, 'utf8')) : new Map();
  const repo = await findRepoRoot(root);

  const nodes = [];
  const edges = [];
  for (const n of graph.allNodes()) {
    const coveragePct = hasLcov ? coverageForRange(lcov, n.file, n.startLine, n.endLine) : null;
  ...
  const summary = `${nodes.length} functions · ${edges.length} edges · ` +
    (hasLcov ? 'lcov coverage' : 'proxy coverage (no lcov.info)') + (repo ? '' : ' · no git history');

  const template = fs.readFileSync(path.join(__dirname, '..', 'media', 'graph.html'), 'utf8');
  const html = buildStandaloneHtml(template, { type: 'graph', nodes, edges, summary }, new Date().toLocaleString());
  ...
  execFile(opener, [out], (err) => { if (err) console.log('open it manually:', out); });
