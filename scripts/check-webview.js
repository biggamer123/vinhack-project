...
function applyDomShims(w) {
  Object.defineProperty(w.HTMLElement.prototype, 'clientWidth', { get: () => 1000, configurable: true });
  Object.defineProperty(w.HTMLElement.prototype, 'clientHeight', { get: () => 700, configurable: true });
  for (const [prop, value] of [['width', 1000], ['height', 700]]) {
...
async function buildPayload() {
  ...
  for (const n of graph.allNodes()) {
    const coveragePct = lcov.size ? coverageForRange(lcov, n.file, n.startLine, n.endLine) : null;
    const h = repo ? await historyForRange(repo, n.file, n.startLine, n.endLine) : null;
    const risk = {
    ...
    nodes.push({
      id: n.id, name: n.name, file: path.relative(root, n.file), startLine: n.startLine,
      fanIn: n.callers.size, fanOut: n.callees.size, score, tier: tierFor(score), risk,
    });
    for (const c of n.callees) edges.push({ from: n.id, to: c });
  }
  nodes.sort((a, b) => b.score - a.score);
  return { type: 'graph', nodes, edges, summary: `${nodes.length} functions`, focus: nodes[0].id };
}
...
(async () => {
  ...
  let html = fs.readFileSync(path.join(__dirname, '..', 'media', 'graph.html'), 'utf8')
  ...
  check('page signals ready to the extension host', posted[0] && posted[0].type, 'ready');

  window.dispatchEvent(new window.MessageEvent('message', { data: payload }));
  await new Promise((r) => setTimeout(r, 400));

  const doc = window.document;
  check('all nodes rendered', doc.querySelectorAll('g.node').length, payload.nodes.length);
  check('all edges rendered', doc.querySelectorAll('line.link').length, payload.edges.length);
  check('subtitle shows the summary', doc.getElementById('subtitle').textContent.includes('FUNCTIONS'));
  check('HUD reports tier counts', doc.getElementById('hud').innerHTML.includes('GRASS'));

  // The focus id from a CodeLens click should already have opened that dex entry.
  const entry = doc.getElementById('entry');
  check('focused function opened its dex entry', entry.querySelector('.mon').textContent, payload.nodes[0].name);
  check('entry shows a tier badge', entry.querySelector('.badge').textContent.length > 0);
  check('entry shows stat bars', entry.querySelectorAll('.statrow').length, 5);
  check('entry shows the score arithmetic', entry.querySelector('.formula').textContent.includes('= ' + payload.nodes[0].score));
  check('callers/callees card rendered', doc.querySelectorAll('aside .card').length, 2);

  // Clicking OPEN IN EDITOR must message the extension host with the node id.
  entry.querySelector('#open').dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
  const reveal = posted.find((m) => m.type === 'reveal');
  check('OPEN IN EDITOR posts a reveal message', reveal && reveal.id, payload.nodes[0].id);

  // Clicking a different node switches the entry.
  const circles = doc.querySelectorAll('g.node circle');
  circles[circles.length - 1].dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
  check('clicking a node switches the dex entry', entry.querySelector('.mon').textContent !== payload.nodes[0].name);

  // Tier filter hides nodes.
  const before = [...doc.querySelectorAll('g.node')].filter((n) => n.style.display !== 'none').length;
  doc.querySelector('.tierchip[data-tier="low"]').dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
  const after = [...doc.querySelectorAll('g.node')].filter((n) => n.style.display !== 'none').length;
  ...
  check('search narrows the view', [...doc.querySelectorAll('g.node')].filter((n) => n.style.display !== 'none').length < after);

  // --- standalone (browser snapshot) mode: host-dependent buttons must disappear ---
  window.dispatchEvent(new window.MessageEvent('message', {
    data: Object.assign({}, payload, { standalone: true, focus: payload.nodes[0].id }),
  }));
  await new Promise((r) => setTimeout(r, 400));
  check('standalone hides OPEN IN BROWSER', doc.getElementById('browser').style.display, 'none');
  check('standalone hides OPEN IN EDITOR', doc.getElementById('open'), null);
  check('standalone still renders the graph', doc.querySelectorAll('g.node').length, payload.nodes.length);

  // centerOn() runs a zoom transition on focus — make sure it actually transformed.
  await new Promise((r) => setTimeout(r, 900));
  check('focus zoomed the canvas', (doc.querySelector('svg > g').getAttribute('transform') || '').includes('translate'));
  ...
  const template = fs.readFileSync(path.join(__dirname, '..', 'media', 'graph.html'), 'utf8');
  const standaloneHtml = buildStandaloneHtml(template, payload, 'test stamp')
    .replace(/<script [^>]*src="https:\/\/cdnjs[^"]*"[^>]*><\/script>/, '');
  check('stub is injected before the page script',
    standaloneHtml.indexOf('acquireVsCodeApi = ()') < standaloneHtml.indexOf("const vscode = acquireVsCodeApi()"));

  const dom2 = new JSDOM(standaloneHtml, { runScripts: 'outside-only', pretendToBeVisual: true });
  ...
  for (const block of standaloneHtml.match(/<script nonce="standalone">([\s\S]*?)<\/script>/g) || []) {
  ...
  check('snapshot page renders its nodes', d2.querySelectorAll('g.node').length, payload.nodes.length);
  check('snapshot page leaves booting state', d2.getElementById('subtitle').textContent.includes('SNAPSHOT'));
  check('snapshot HUD leaves LOADING state', !d2.getElementById('hud').innerHTML.includes('LOADING'));
  check('snapshot hides OPEN IN BROWSER', d2.getElementById('browser').style.display, 'none');
