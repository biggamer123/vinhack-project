...
/** One service depending on another, by service id. Drawn as an arrow in the builder. */
...
export interface Stack {
  ...
  /**
   * Which services talk to which. Absent means "whatever the shapes suggest"
   * (see defaultLinks); once the user edits a connection the list is stored, so
   * two backends can use different databases.
   */
  links?: StackLink[];
}
...
const DATA_KINDS = ["database", "cache", "queue"];

/** The obvious wiring: apps to their data, frontends to an API, a proxy to everything it fronts. */
export function defaultLinks(services: StackService[]): StackLink[] {
  const kind = (s: StackService) => templateById(s.template)?.kind;
  ...
  for (const from of services) {
    for (const to of services) {
      ...
      const b = kind(to);
      const linked =
        (a === "backend" && DATA_KINDS.includes(b || "")) ||
        (a === "frontend" && b === "backend") ||
        (from.template === "nginx" && (b === "frontend" || b === "backend")) ||
        (from.template === "adminer" && (to.template === "postgres" || to.template === "mysql"));
      if (linked) {
...
/** The connections in force: the user's own, or the defaults while they have not touched any. */
export function linksOf(stack: Stack): StackLink[] {
  const ids = new Set(stack.services.map((s) => s.id));
  return (stack.links ?? defaultLinks(stack.services)).filter((l) => ids.has(l.from) && ids.has(l.to) && l.from !== l.to);
}
...
export function generateStack(stack: Stack): GeneratedStack {
  ...
  // Connections drive both depends_on and the environment variables an app gets.
  const links = linksOf(stack);
  const linkedFrom = (s: StackService) =>
    links.filter((l) => l.from === s.id).map((l) => services.find((x) => x.id === l.to)).filter(Boolean) as StackService[];

  // A database nothing connects to still starts, but no service can reach it.
  for (const d of data) {
    if (!links.some((l) => l.to === d.s.id)) {
      warnings.push(`Nothing is connected to ${d.s.name} - no service gets its connection details.`);
    }
  }
  ...
  for (const s of services) {
    ...
    if (t.kind === "frontend" || t.kind === "backend") {
      ...
      if (t.kind === "backend") {
        ...
        const myData = data.filter((d) => linkedFrom(s).some((x) => x.id === d.s.id));
      ...
      const myApi = linkedFrom(s).filter((x) => templateById(x.template)!.kind === "backend");
      if (t.kind === "frontend" && myApi.length) {
      ...
      const upstream = linkedFrom(s).filter((u) => ["frontend", "backend"].includes(templateById(u.template)!.kind));
      if (upstream.length) {
        out.push(`    depends_on:`);
        for (const u of upstream) out.push(`      - ${u.name}`);
      }
      const upFront = upstream.filter((u) => templateById(u.template)!.kind === "frontend")[0] || frontends[0];
      const upApi = upstream.filter((u) => templateById(u.template)!.kind === "backend")[0] || backends[0];
      files.push({ path: "nginx/nginx.conf", language: "nginx", service: s.name, content: nginxProxyConf(upFront, upApi, stack.mode) });
      ...
      const browsing = linkedFrom(s).filter((d) => d.template === "postgres" || d.template === "mysql");
      if (browsing.length) {
        out.push(`    environment:`, `      ADMINER_DEFAULT_SERVER: ${browsing[0].name}`);
        out.push(`    depends_on:`);
        for (const d of browsing) out.push(`      - ${d.name}`);
      } else if (sqlDbs.length) {
        warnings.push(`${s.name} is not connected to a database - drag a connection to one.`);
      } else {
