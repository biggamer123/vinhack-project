...
export class GraphPanel {
  ...
  private devopsData: DevopsPayload | null = null;
  /** Tab to switch to on the next update, e.g. when opened from the DevOps command. */
  focusTab: string | undefined;
  ...
  /** Open the panel straight on the DEVOPS tab. */
  static showDevops(context: vscode.ExtensionContext, graph: CallGraph, risk: RiskService): void {
    const panel = GraphPanel.show(context, graph, risk);
    panel.focusTab = "devops";
    panel.devopsData = devopsFor();
    panel.update();
  }

  static refreshDevopsIfOpen(): void {
    GraphPanel.current?.refreshDevops();
  }

  refreshDevops(): void {
    this.devopsData = devopsFor();
    this.update();
  }
  ...
  private onMessage(msg: {
    ...
    if (msg.type === "browser") {
      void openInBrowser(this.context, this.graph, this.risk, msg.id, {
        focusTab: msg.tab,
        devopsMode: msg.devopsMode,
        builderStack: msg.stack,
      });
      return;
    }
    if (msg.type === "schema") {
    ...
    if (msg.type === "devopsOpen" && msg.file) {
      const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (root) {
        void this.revealTest(path.join(root, msg.file), msg.line);
      }
      return;
    }
    if (msg.type === "devopsWrite" && Array.isArray(msg.files)) {
      void writeGeneratedFiles(msg.files);
      return;
    }
  ...
  private html(): string {
    ...
    void file;
    return loadPageTemplate(this.context.extensionPath)
...
/** Analyse the open workspace's Docker setup. */
export function devopsFor(): DevopsPayload | null {
  const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  if (!root) {
    return null;
  }
...
/**
 * Write files generated by the DevOps builder. Existing files are never
 * overwritten without asking; the user picks overwrite or skip for them.
 */
export async function writeGeneratedFiles(files: { path: string; content: string }[]): Promise<void> {
  const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  if (!root) {
    vscode.window.showWarningMessage("Blast Radius: open a folder first, then write the Docker files into it.");
    return;
  }
  const safe = files.filter((f) => {
    const target = path.resolve(root, f.path);
    return target.startsWith(path.resolve(root) + path.sep); // never outside the workspace
  });
  const existing = safe.filter((f) => fs.existsSync(path.join(root, f.path)));
  let overwrite = false;
  if (existing.length) {
    const choice = await vscode.window.showWarningMessage(
      `${existing.length} of these files already exist: ${existing.map((f) => f.path).join(", ")}`,
      { modal: true },
      "Overwrite them",
      "Skip existing files",
    );
    ...
    overwrite = choice === "Overwrite them";
  }
  let written = 0;
  for (const f of safe) {
  ...
  if (fs.existsSync(compose)) {
    await vscode.window.showTextDocument(vscode.Uri.file(compose), { preview: false, viewColumn: vscode.ViewColumn.One });
  }
  vscode.window.showInformationMessage(`Blast Radius: wrote ${written} file${written === 1 ? "" : "s"}. Follow DOCKER.md to start.`);
  GraphPanel.refreshDevopsIfOpen();
}
...
export async function openInBrowser(
  ...
  risk: RiskService,
  focusId?: string,
  view: { focusTab?: string; devopsMode?: string; builderStack?: unknown } = {},
): Promise<void> {
  // With no code indexed there is no graph, but the Docker builder is still
  // useful - an empty folder is exactly where it is needed - so open on it.
  if (graph.size === 0 && !view.focusTab) {
    view = { ...view, focusTab: "devops", devopsMode: "builder" };
  }
    ...
    async (progress) => {
      try {
        ...
        const payload = {
          ...
          devopsMode: view.devopsMode,
          // The builder's stack lives in the webview's storage, which a browser tab
          // cannot read, so it travels with the snapshot.
          builderStack: view.builderStack,
        };

        const template = loadPageTemplate(context.extensionPath);
