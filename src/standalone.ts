...
export function buildStandaloneHtml(template: string, payload: StandalonePayload, stamp: string): string {
  ...
  if (!D3_TAG.test(template)) {
    throw new Error('graph.html: could not find the d3 script tag to inject before');
  }

  const bootstrap =
    `<script nonce="standalone">
